...
#  (1) EC      | Random (no FS*) | Equalization (2) | Monogamy | Yes              | No       |
...
rm script_extgwas_PURGd2.1_zyg_kn.sh.*
...
PFS=$4		#proportion partial full-sib mating
SEED=$5
...
#PURGd
Edelta=0	#0:estimation of delta from ind.Fa=0, 1:at the same time as any other parameter
...
#CAMBIAR
mkdir -p $WDIR/extgwas_PURGd_Results/k2n_L$LAMB.K$NCRO.s$AVEs.h$AVEh.N$NMAX.Edelta$Edelta/$case.$fit.mfec$MFEC
DIR="extgwas_PURGd_Results/k2n_L$LAMB.K$NCRO.s$AVEs.h$AVEh.N$NMAX.Edelta$Edelta/$case.$fit.mfec$MFEC"
...
cp extinctiongwas_ms_zyg_kn $SCDIR/$SLURM_JOBID/
...
time ./extinctiongwas_ms_zyg_kn>>out<<@
0
$seed
$NMAX	NMAX
$MFEC	MAXFEC
...
if [[ "$Edelta" == 1 ]] ; then	./PURGd --seed=1234 --config=settings.txt pedfile${i}.csv
...
if [[ "$FV" < 2 ]] ; then
	if [[ "$Edelta" == 1 ]] ; then	./PURGd --seed=1234 --config=settings.txt fec_pedfile${i}.csv
	else	./PURGd --seed=1234 --delta=s fec_pedfile${i}.csv
	fi
	if [[ "$Edelta" == 1 ]] ; then	./PURGd --seed=1234 --config=settings.txt via_pedfile${i}.csv
